...
/*   Updated: 2018/03/22 14:50:12 by ysingaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_direct_param(char *str, t_param *param, t_champ *champ, t_cmd *cmd)
{
	...
	if (str[0] == LABEL_CHAR)
		...
		set_int_param(str, param, champ, DIR_CODE);
}
...
t_param	*new_param(char *str, t_champ *champ, t_cmd *cmd)
{
	...
	else if (str[i] == LABEL_CHARS)
		set_label_param(&str[i + 1], param, champ, IND_CODE);
	else
		set_int_param(str, param, champ, IND_CODE);
}
